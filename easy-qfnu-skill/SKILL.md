...
python3 scripts/qfnu jwxt login --username "$QFNU_JWXT_USERNAME" --password "$QFNU_JWXT_PASSWORD"
...
- If OCR is down, tell the user to run `python3 scripts/setup_ocr` then `python3 scripts/run_ocr`. Do not solve captchas by vision in the agent.
...
- JWXT login needs a local ddddocr Flask service (`http://127.0.0.1:5000` by default) and one Cookie jar. Setup: `scripts/setup_ocr` (uv if present, else pip) then `scripts/run_ocr`.
