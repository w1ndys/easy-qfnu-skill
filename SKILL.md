...
- The public cached pre-course catalog is 夫子校园’s read-only scheduled snapshot and requires no JWXT login. Use the CLI command only after the user agrees to this fallback; never request or expose an upstream API key; provide at least one non-empty condition and always say it is a 后台定时缓存 with delay, not live enrollment data.
